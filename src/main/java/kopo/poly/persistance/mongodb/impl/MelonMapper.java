...
public class MelonMapper extends AbstractMongoDBComon implements IMelonMapper {
	...
	public List<MelonDTO> getSingerSong(String pColNm, String pSinger) throws Exception {
		...
		//조회할 조건(SQL의 WHERE 역활 / SELECT song, singer FROM MELON_000000 where singer ='(여자)아이들'
		...
		for (Document doc : rs) {
			...
			//조회 잘되나 출력해 봄
			String song = CmmUtil.nvl(doc.getString("somg"));
			String singer = CmmUtil.nvl(doc.getString("singer"));
	...
	public int updateSong(String pColNm, String pSinger, String pUpdateSinger) throws Exception {
		...
		//조회할 조건(SQL의 WHERE 역활 / SELECT * FROM MELON_20220405 where singer = '방탄소년단')
		Document query = new Document();
		query.append("singer", pSinger);

		//MongoDB 데이터 삭제는 반드시 컬렉션을 조회하고, 조회된 ObjectID를 기반으로 데이터를 삭헤함
		//MongoDB 환경은 분산환경으로 구성될 수 있기 떄문에 정확한 pk에 매핑하기 위해서임
		FindIterable<Document> rs = col.find();

		//람다식 활용하여 데이터 삭제하기
		//전체 컬렉션에 있는 데이터를 삭제하기;
	...
	public int dropMelonCollection(String colNm) throws Exception {
		...
		log.info(this.getClass().getName() + ".dropMelonCollection Strat");
	...
	public int updateSongAddListField(String pColNm, String pSinger, List<String> pMember) throws Exception {
		...
		//MongoDB 데이터 삭제는 반드시 컬렉션을 조회하고 조회된 데이터를 삭제함
		//MongoDB 환경은 분산환경 으로 구성될 수 있기 때문에 정확한 pk에 매핑하기 위해서임
		FindIterable<Document> rs = col.find(query);

		//람다식 활용하여 member 필드 추가
